...
icon="$HOME/.config/dunst/icons/screenshot.svg"
...
countdown () {
	for sec in `seq $1 -1 1`; do
                #notify-send.sh "Taking shot : $sec" -i "$icon" -t 2000 --replace=555
		dunstify -t 1000 --replace=699 -i $icon "Taking shot in : $sec"
		sleep 1
	done
}
